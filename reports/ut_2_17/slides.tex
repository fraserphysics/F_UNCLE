...
\title{The F\_UNCLE Project: Functional UNcertainty Constrained by Law
  and Experiment\footnote{LA-UR-17-20559}}
...
\frame{
  ...
  \end{description}
  Also need $C_V$.  From now on, I focus on $p(V)$.
}
\frame{
  \frametitle{Constrained Optimization}
  %
  \begin{description}
  \item[Constraints] As a function of volume, require pressure to be
    \begin{itemize}
    \item Positive
    \item Monotonic
    \item \textbf{Convex}
  ...
  \item[Cost] Weighted sum of squares or
    $C(\theta) = \log(z)-\log(\text{a posteriori probability})$
  \end{description}
    \begin{align*}
      C(\theta) &= - \frac{1}{2} \left(\theta - \mu_\theta \right)^T
                   \Sigma_\theta^{-1}  \left(\theta - \mu_\theta \right)  &&
                   \text{Log prior} \\
      &\quad
                  -\frac{1}{2} \sum_i \left(x_i - \mu_i(\theta)
                  \right)^T \Sigma_i^{-1} \left(x_i - \mu_i(\theta)
                  \right) && \text{Log likelihood} \\        
      &\equiv - \LL_0 -\sum_i \LL_i
    \end{align*}
}
